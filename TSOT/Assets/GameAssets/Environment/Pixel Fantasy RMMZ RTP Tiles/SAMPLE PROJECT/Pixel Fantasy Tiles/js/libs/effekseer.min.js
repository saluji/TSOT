...
if(!this.isLoaded&&loaded){this.isLoaded=true;if(this.onload)this.onload();}}
},{key:"getColorImagePaths",value:function getColorImagePaths(){var arr=[];var count=Core.EffectGetColorImageCount(this.nativeptr);for(var i=0;i<count;i++){var _ptr=Core.EffectGetColorImagePath(this.nativeptr,i);str=Module.UTF8ToString(_ptr);arr.push(str);}
return arr;}}]);return EffekseerEffect;}();var EffekseerHandle=function(){function EffekseerHandle(context,native){_classCallCheck(this,EffekseerHandle);this.context=context;this.native=native;}
...
},{key:"setDynamicInput",value:function setDynamicInput(index,value){Core.SetDynamicInput(this.context.nativeptr,this.native,index);}
...
this.gl.bindBuffer(this.gl.ARRAY_BUFFER,this.current_vbo);this.gl.bindBuffer(this.gl.ELEMENT_ARRAY_BUFFER,this.current_ibo);}}]);return ContextStates;}();var EffekseerContext=function(){function EffekseerContext(){_classCallCheck(this,EffekseerContext);}
_createClass(EffekseerContext,[{key:"_loadBinary_with_effect_cache",value:function _loadBinary_with_effect_cache(path,effect,onload,onerror){var res=effect.resources.find(function(res){return res.path==path;});if(res){onload();return res.isLoaded?res.buffer:null;}
var res={path:path,isLoaded:false,buffer:null,isRequired:true};effect.resources.push(res);_loadBinFile(effect.baseDir+path,function(buffer){res.buffer=buffer;res.isLoaded=true;onload(buffer);},onerror);return null;}},{key:"_getglTFBodyURI",value:function _getglTFBodyURI(buffer){var memptr=Module._malloc(buffer.byteLength);Module.HEAP8.set(new Uint8Array(buffer),memptr);ptr=Core.GetglTFBodyURI(context.nativeptr,memptr,buffer.byteLength);str=Module.UTF8ToString(ptr);Module._free(memptr);return str;}},{key:"_isBinaryglTF",value:function _isBinaryglTF(context,buffer){var memptr=Module._malloc(buffer.byteLength);Module.HEAP8.set(new Uint8Array(buffer),memptr);var ret=Core.IsBinaryglTF(context.nativeptr,memptr,buffer.byteLength);Module._free(memptr);return ret>0;}},{key:"_makeContextCurrent",value:function _makeContextCurrent(){Module.GL.makeContextCurrent(this.ctx);}
},{key:"init",value:function init(webglContext,settings){this.gl=webglContext;this.contextStates=new ContextStates(this.gl);window.gl=this.gl;this.ctx=Module.GL.registerContext(webglContext,{majorVersion:1,minorVersion:0,enableExtensionsByDefault:true});this._makeContextCurrent();if(!settings){settings={instanceMaxCount:4000,squareMaxCount:10000};}
this.contextStates.save();this.nativeptr=Core.Init(settings.instanceMaxCount,settings.squareMaxCount);this.contextStates.restore();}
...
},{key:"draw",value:function draw(){this._makeContextCurrent();var program=this.gl.getParameter(gl.CURRENT_PROGRAM);this.contextStates.save();Core.Draw(this.nativeptr);this.contextStates.restore();this.gl.useProgram(program);}},{key:"beginDraw",value:function beginDraw(){this.contextStates.save();Core.BeginDraw(this.nativeptr);}},{key:"endDraw",value:function endDraw(){Core.EndDraw(this.nativeptr);this.contextStates.restore();}},{key:"drawHandle",value:function drawHandle(handle){Core.DrawHandle(this.nativeptr,handle.native);}
...
},{key:"loadEffect",value:function loadEffect(path){var scale=arguments.length>1&&arguments[1]!==undefined?arguments[1]:1.0;var onload=arguments[2];var _this=this;var onerror=arguments[3];var redirect=arguments[4];this._makeContextCurrent();var effect=new EffekseerEffect(this);var dirIndex=path.lastIndexOf("/");if(typeof scale==="function"){console.log("Error : second arguments is number from version 1.5");effect.scale=1.0;effect.onload=scale;effect.onerror=onload;effect.redirect=redirect;}else{effect.scale=scale;effect.onload=onload;effect.onerror=onerror;effect.redirect=redirect;}
if(typeof path==="string"){effect.baseDir=dirIndex>=0?path.slice(0,dirIndex+1):"";_loadBinFile(path,function(buffer){if(_this._isBinaryglTF(_this,buffer)){bodyPath=_this._getglTFBodyURI(buffer);_this._loadBinary_with_effect_cache(bodyPath,effect,function(bufferBody){effect._load(buffer);},effect.onerror);}else{effect._load(buffer);}},effect.onerror);}else if(typeof path==="arraybuffer"){var buffer=path;effect._load(buffer);}
return effect;}
...
},{key:"isVertexArrayObjectSupported",value:function isVertexArrayObjectSupported(){return Core.IsVertexArrayObjectSupported(this.nativeptr);}}]);return EffekseerContext;}();var Effekseer=function(){function Effekseer(){_classCallCheck(this,Effekseer);}
